$(function(){
  ...
  $('.video-show').click(function(){
    $('.img-txt-nav ul li a').removeClass("nav-hign-light");
    $(this).addClass('nav-hign-light');
    $('.special-imgTxt').removeClass('part-active');
    $('.usercenter-video-banner').addClass('part-active')
  })
  $('.imgTxt-show').click(function(){
    $('.img-txt-nav ul li a').removeClass("nav-hign-light");
    $(this).addClass('nav-hign-light');
    $('.usercenter-video-banner').removeClass('part-active');
    $('.special-imgTxt').addClass('part-active')
  })
  /* 举报框JS */
  $('.report-save-btn').click(function(){
   ...
   $('.tag-info-detail div').each(function(){
    var max_width=140;
    if ($(this).text().length>max_width) {
      $(this).text($(this).text().substring(0,max_width));
      $(this).html($(this).html()+'...');
   ...
   $('.load-more').click(function(){
        $(this).before('<li><img src="images/tag-img/tag-img-1.jpg" alt="tag-img-1"><a href="javascript:void(0);">今夜大风卷过沙地，恍惚之间我看到了千年古城一刹那的风情</a></li>')
   })
   $('.usercenter-img-mask').find('a').click(function(event){
